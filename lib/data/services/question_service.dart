...
class QuestionService {
  ...
  Future<List<QuestionModel>> getQuestions() async {
    try {
      final response = await _client
          .from('questions')
          .select('*, users(full_name), question_tags(tags(name)), answers(count)')
  ...
  Future<void> incrementViewCount(String questionId) async {
    try {
      ...
      try {
        final resp = await _client.from('questions').select('view_count').eq('id', questionId).single();
  ...
  Future<List<AnswerModel>> getAnswers(String questionId) async {
    try {
      ...
      return (response as List).map((json) => AnswerModel.fromJson(json)).toList();
    } catch (e) {
  ...
  Future<void> createQuestion(String authorId, String title, String content, List<String> tags) async {
    try {
      final result = await _client.from('questions').insert({
        'author_id': authorId,
        'title': title,
        'content': content,
      }).select().single();
      ...
      for (final tag in tags) {
        ...
        final tagResult = await _client.from('tags').upsert({'name': tag}).select().single();
  ...
  Future<void> postAnswer(String questionId, String authorId, String content) async {
    try {
  ...
  Future<void> vote(String targetId, String userId, bool isQuestion, int voteValue) async {
    try {
      ...
      await _client.from('votes').upsert(
        data,
        onConflict: 'user_id, $targetField',
      );

      // Manual increment fallback if RPC/Triggers aren't setup
      final column = voteValue == 1 ? 'upvotes' : 'downvotes';
      try {
        await _client.rpc('increment_score_v2', params: {
           'target_table': table,
           'target_id': targetId,
           'column_name': column,
           'amount': 1
         ...
         // Second fallback: direct update (not atomic but better than nothing)
         try {
           final response = await _client.from(table).select(column).eq('id', targetId).single();
